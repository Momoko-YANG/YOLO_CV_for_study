# -*- coding: utf-8 -*-
"""
Main window for gesture recognition system (without login).
"""
...
from utils.strings import get_string, set_language
...
class RecMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def toggle_table_review(self, row, col):
        ...
        try:
            ...
            logger.error(f"Error in toggle_table_review: {e}")
            self.label_display.setText(get_string('cannot_reproduce'))
    ...
    def handle_image_opened(self):
        ...
        if os.path.isfile(self.file_path):  # 检查文件路径是否为文件
            ...
            logger.warning(get_string('invalid_path'))
    ...
    def handle_image_failed(self, error):
        ...
        - 在控制台打印错误信息
        """
        logger.error(f"Failed to open image file: {error}")
    ...
    def handle_video_failed(self, error):
        ...
        - 在控制台打印错误信息
        """
        logger.error(f"Failed to open video file: {error}")
